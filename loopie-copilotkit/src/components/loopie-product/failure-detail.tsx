...
export function FailureDetail({ failureId }: { failureId: string }) {
  ...
  async function propose() {
    ...
    try {
      ...
      setMessage(response.ok ? payload.shadow_passed ? `Correction ${payload.id} is ready for human review.` : `Correction ${payload.id} failed its shadow gate; no artifact changed.` : payload.detail ?? payload.error ?? "Correction generation failed");
  ...
  return <>
    ...
    <section className="lp-card"><button className="lp-button" disabled={busy || row.status !== "open"} aria-busy={busy} onClick={() => void propose()}>{busy ? <><span className="lp-button-spinner" aria-hidden="true" />Generating and testing</> : "Generate and shadow-test correction"}</button>{message ? <p>{message}</p> : null}{message ? <Link href="/corrections">Open correction records →</Link> : null}</section>
